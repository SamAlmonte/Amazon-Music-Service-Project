...
public class GetPlaylistSongsActivityTest {
    ...
    void handleRequest_withShuffledSongOrder_returnsSongsInAnyOrder() {
        ...
        for (int i =0; i < playlist.getSongList().size(); i++) {
            songModels.add(SongModel.builder().withTitle(playlist.getSongList().get(i).getSongTitle()).withAlbum(playlist.getSongList().get(i).getAlbumName()).withAsin(playlist.getSongList().get(i).getAsin()).withTrackNumber(playlist.getSongList().get(i).getTrackNumber()).build());
        }
